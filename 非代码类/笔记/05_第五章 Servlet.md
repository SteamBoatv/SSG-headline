...
*HttpServletRequest中包含了请求行/请求头/请求体，也就是请求的全部信息。*

*随后在service中进行java计算。*

*最后把计算好的数据放入HttpServletResponse，传出去。*
...
+ HttpServletRequest 代表请求对象,是有请求报文
...
<web-app xmlns="https://jakarta.ee/xml/ns/jakartaee"
    ...
    <servlet>
        ...
        <!--路径从src之后开始算-->
        <servlet-class>com.atguigu.servlet.UserServlet</servlet-class> <!--路径-->
    ...
    <servlet-mapping>
        <!--关联别名和映射路径-->
        <servlet-name>userServlet</servlet-name>
        <!--可以为一个Servlet匹配多个不同的映射路径,但是不同的 Servlet不能使用相同的url-pattern-->
        <url-pattern>/userServlet</url-pattern> <!--前端的请求路径-->⭐ 前端界面写这个，action之后写这个！！
        
        <!--了解即可 几乎不会用
            /        表示通配所有资源,不包括jsp文件，    web01/XX/XX 写什么都会请求到此Servlet
            /*       表示通配所有资源,包括jsp文件 ，     写什么都会访问到Servlet
            /a/*     匹配所有以a前缀的映射路径           web01/a/XX/XX 会匹配到Servlet
            *.action 匹配所有以action为后缀的映射路径 	web01/xxx/aa/vv/asd.action （不是POST和GET来的）
        -->
...
+ **一个servlet可以对应多个不同的url-pattern**（前端搜索栏访问地址）
+ 多个servlet不能使用相同的url-pattern
+ url-pattern中可以使用一些通配写法 (了解即可，不重要)
    + /        表示通配所有资源,不包括jsp文件
    + /*      表示通配所有资源,包括jsp文件
    + /a/*     匹配所有以a前缀的映射路径
...
为什么在请求 servlet的时候，没有content-type(默认为了text/html)

在tomcat10里面的conf目录中有web.xml。里面说明了遇到某种后缀文件，生成xxx的content-type。

手动给servlet设置头：`resp.setHeader("Content-Type","text/html");`

```
⭐resp.setContentType("text/html"); 专属简化设置方法
```
...
``` java
@WebServlet(
        name = "userServlet", //给Servlet起别名，意义不大
        //value = "/user",  //前端的地址 一直说的那个 /s1
        urlPatterns = {"/userServlet1","/userServlet2","/userServlet"},  //前端的地址，多个路径，多个请求路径
        initParams = {@WebInitParam(name = "encoding",value = "UTF-8")},  //config对象
        loadOnStartup = 6   //生命周期
)
public class UserServlet  extends HttpServlet {
    @Override
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
...
-   简单的叙述生命周期，就是**对象在容器中从开始创建到销毁的过程**。

> Servlet容器

+ Servlet对象是Servlet容器创建的，生命周期方法都是由容器(目前我们使用的是Tomcat)调用的。这一点和我们之前所编写的代码有很大不同。在今后的学习中我们会看到，越来越多的对象交给容器或框架来创建，越来越多的方法由容器或框架来调用，开发人员要尽可能多的将精力放在**业务逻辑**的实现上。
...
``` xml
    ...
    <servlet>
        ...
        <!--load-on-startup
            ...
            数字表示的是Servlet的实例化的顺序,假设有1个Servlet,你写个15,他还是第一个实例化  -1表示tomcat不会帮助实例化
			如果序号冲突,tomcat会自动解决冲突
			序号不用考虑是否连贯
			tomcat默认写到了5.自己写从6开始写
        -->
...
1. ⭐Servlet内部的成员变量是共享的，**不可以在Servlet中修改成员变量**，会有线程安全问题.
...
+ Servlet 规范接口,所有的Servlet必须实现 
    + public void **init**(ServletConfig config) throws ServletException;   
        + 初始化方法,容器在构造servlet对象后,tomcat自动调用的方法,容器负责实例化一个ServletConfig对象,并在调用该方法时传入
...
## 5.2 GenericServlet 抽象类 , 实现了Servlet接口 

侧重除了`service`方法以外的方法处理
...
+ GenericServlet 抽象类是对Servlet接口一些固定功能的粗糙实现,以及对service方法的再次抽象声明,并定义了一些其他相关功能方法
    ...
    + public GenericServlet() { 空} 
        + 构造器,为了满足继承而准备
        
    + public void **destroy**() { 空 } 
        + 销毁方法的平庸实现 (重写了,但是里面还是为空)
...
+ 为Servlet提供**初始配置参数**的一种对象,每个Servlet都有自己独立唯一的ServletConfig对象
...
//        获取config 配置信息
        ...
        String keysA = config.getInitParameter("keysA");
        System.out.println("keysA is : " + keysA);

//        循环获取键值对的 key值
        Enumeration<String> initParameterNames = config.getInitParameterNames();
//        hasMoreElements 判断是否有下一个
        while (initParameterNames.hasMoreElements()) {
//            nextElement向下移动游标一次 并返回
            String s = initParameterNames.nextElement();
            String key = config.getInitParameter(s);
...
+ 在Servlet中获取ServletContext并获取参数
  + ⭐思路和config一样的,代码也是一样的.不同的地方有两个：
  + 1. 获取的时候,由`getServletConfig();`变为：`getServletContext();`。
    2. 在获取迭代对象的时候，调用者不同`config.getInitParameterNames();`，`context.getInitParameterNames();`
...
例如： servletContext.getRealPath("servlet"); <- src/servlet
    "E:\IntelliJFile\SSG-Web-ALL\out\artifacts\demo02_servlet_war_exploded\servlet"
```

+ 例如我们的目标是需要获取项目中某个静态资源的路径，不是工程目录中的路径，而是**部署目录中的路径**；我们如果直接拷贝其在我们电脑中的完整路径的话其实是有问题的，因为如果该项目以后部署到公司服务器上的话，路径肯定是会发生改变的，所以我们需要使用代码动态获取资源的真实路径.  
...
+ 域对象: 一些用于存储数据和传递数据的对象,传递数据**不同的范围**,我们称之为**不同的域**,不同的域对象代表不同的域,共享数据的范围也不同
+ `ServletContext`代表应用,所以`ServletContext`域也叫作应用域,**是webapp中最大的域**,可以在**本应用内**实现数据的共享和传递
+ webapp中的三大域对象,分别是应用域,会话域,请求域
+ `后续我们会将三大域对象统一进行讲解和演示`,三大域对象都具有的API如下

| API                                         | 功能解释                |
| ------------------------------------------- | ----------------------- |
| void setAttribute(String key,Object value); | 向域中存储/**修改**数据 |
...
|                               | http://localhost:8080/demo02/RequestTest                     |
| String getRequestURI();       | 获取客户端请求项目中的具体资源                               |
|                               | /demo02/RequestTest                                          |
| int getServerPort();          | 获取客户端发送请求时的端口    客户端---**1234**--->代理-----8080----->tomcat |
|                               | 8080                                                         |
| int getLocalPort();           | 获取本应用在所在容器的端口 tomcat发出去的                    |
|                               | 8080                                                         |
| int getRemotePort();          | 获取**客户端**程序的端口                                     |
|                               | 14714                                                        |
...
+ 获得请求参数相关 (POST 和 GET 都是同样的方法 不受影响)(信息无论是在请求头中还是请求体中)
| API                                                     | 功能解释                                                |
| ------------------------------------------------------- | ------------------------------------------------------- |
| 专门获取**KEY-VALUE**格式                               |                                                         |
| String getParameter(String parameterName);              | 根据请求参数名获取请求单个参数值                        |
| String[] **getParameterValues**(String parameterName);  | 根据请求参数名获取请求多个参数值数组(checkbox)          |
|                                                         | 其实特殊的就这一个，其他的和之前的config，context一样的 |
| Enumeration<String> getParameterNames();                | 获取所有请求参数名                                      |
| Map<String, String[]> getParameterMap();                | 获取所有请求参数的键值对集合                            |
| 专门获取 **VALUE** 格式 （JSON/文件等）                 |                                                         |
| BufferedReader getReader() throws IOException;          | 获取读取请求体的字符输入流  读JSON                      |
| ServletInputStream getInputStream() throws IOException; | 获取读取请求体的字节输入流 读二进制文件                 |
...
```
        ...
        // entrySet是什么？ entryset是一堆键值对的集合 [{key1:value1},{key2:value2},{key3:value3}]
...
| API                                                    | 功能解释                                         |
| ------------------------------------------------------ | ------------------------------------------------ |
| void setHeader(String headerName, String headerValue); | 设置/**修改**响应头键值对                        |
| void setContentType(String contentType);               | 设置content-type响应头及响应字符集(设置MIME类型) |
| void setContentLength(int length)                      | 设置响应头长度，不能随便写，要设置成恰好的值     |

+ 设置响应体相关 （存返回值）

| API                                                       | 功能解释                                                  |
| --------------------------------------------------------- | --------------------------------------------------------- |
| PrintWriter getWriter() throws IOException;               | 获得向响应体**放入信息**的**字符**输出流                  |
| ServletOutputStream getOutputStream() throws IOException; | 获得向响应体放入信息的**字节**（二进制）输出流 用来放文件 |
|                                                           |                                                           |
...
1. 需要传参数 ?money=1000
2. 要访问 WEB-INF内的资源，只能请求转发

在实现 ”页面跳转“ 的时候，优先使用 响应重定向

## 9.1 概述

> 什么是请求转发和响应重定向

+ 请求转发和响应重定向是web应用中**间接访问**项目资源的两种手段,也是Servlet控制页面跳转的两种手段

+ 请求转发通过HttpServletRequest实现,响应重定向通过HttpServletResponse实现

+ 请求转发（源头不用重新找，等着接就行）生活举例: 张三找李四借钱,李四没有,李四找王五,让王五借给张三
+ 响应重定向（源头重新找）生活举例:张三找李四借钱,李四没有,李四让张三去找王五,张三自己再去找王五借钱
...
+ 请求转发通过**HttpServletRequest对象获取**请求转发器实现
+ 请求转发是服务器内部的行为,**对客户端是屏蔽的**
+ 客户端只发送了**一次请求**,客户端地址栏不变
+ 服务端只产生了一对请求和响应对象,这一对请求和响应对象会继续传递给下一个资源
+ 因为**全程只有一个HttpServletRequset对象**,所以请求参数可以传递,请求域中的数据也可以传递
+ 请求转发可以转发给其他Servlet动态资源,也可以转发给一些静态资源以实现页面跳转
+ 请求转发<u>可以转发给WEB-INF下受保护</u>的资源
  + 也是WEB-INF的访问（目前）唯一途径
...
+ 响应重定向通过<u>HttpServletResponse对象</u>的sendRedirect方法实现
+ 响应重定向是服务端通过**302**响应码和路径,告诉客户端自己去找其他资源,是在服务端提示下的,客户端的行为
+ 客户端至少发送了两次请求,客户端地址栏是要变化的
+ 服务端产生了多对请求和响应对象,且请求和响应对象不会传递给下一个资源
+ 因为全程产生了多个HttpServletRequset对象,所以**请求参数不可以传递**（比如?money=1000）,请求域中的数据也不可以传递
+ 重定向可以是其他Servlet动态资源,也可以是一些静态资源以实现页面跳转
+ 重定向**不可以到给WEB-INF下**受保护的资源
+ 重定向**可以到本项目以外**的外部资源

![image-20231101132738840 style="zoom:30%;"](E:\B站大学\JavaWeb尚硅谷\笔记\05_第五章 Servlet.assets\image-20231101132738840.png)
...
+ GET方式提交时,会对数据<u>使用GBK方式</u>进行URL编码处理 ,是将GBK 转码为 "百分号码"
...
+ 方式2 : 设置Tomcat10.1.7的URI解析字符集和GET请求发送时所使用URL转码时的字符集一致即可,修改conf/server.xml中 Connecter 添加 URIEncoding="GBK"  (不推荐)
  + ⭐此处的 `URIEncoding="GBK"`是设置的如何解析搜索栏中的内容。所以对get生效，但是对post不生效，因为post是放在请求体中，而不是搜索栏中。所以修改无关.
...
+ 方式2 : 后端在获取参数前,设置**解析请求体**使用的字符集和请求发送时使用的字符集一致 (不推荐)
...
![image-20231101150548307](E:\B站大学\JavaWeb尚硅谷\笔记\05_第五章 Servlet.assets\image-20231101150548307.png)

这些默认都是 UTF-8,出现乱码了再排查是哪个的问题
...
+ 在Tomcat10.1.7中,向响应体中<u>放入</u>的数据默认使用了工程编码 UTF-8
+ 浏览器在<u>接收</u>响应信息时,使用了不同的字符集或者是不支持中文的字符集就会出现乱码
...
⭐方式3: 通过设置content-type响应头<u>,告诉浏览器以指定的字符集解析响应体</u>(推荐)



![image-20231101151111646](E:\B站大学\JavaWeb尚硅谷\笔记\05_第五章 Servlet.assets\image-20231101151111646.png)
...
+ 寻找方式就是在当前资源所在路径(http://localhost:8080/web03_war_exploded/)后**拼接**src属性值(static/img/logo.png),正好是目标资源正常获取的url(http://localhost:8080/web03_war_exploded/static/img/logo.png)
...
+ 项目的上下文路径是可以随意变化的
+ ⭐<u>一旦项目的上下文路径发生变化,所有base标签中的路径都需要改</u>

> 解决方案

+ 将项目的上下文路径进行缺省设置,设置为 /,所有的绝对路径中就不必填写项目的上下文了,直接就是/开头即可

### 10.2.2 重定向中的路径问题(只要能用重定向，就不用请求转发)
...
### 10.2.3 请求转发中的路径问题(不用加上下文/web)

⭐**请求转发的 “/” 开头，表示的已经包含上下文了 也就是 http:127.0.0.1:8080/web01/**a
...
![image-20231101195557612](E:\B站大学\JavaWeb尚硅谷\笔记\05_第五章 Servlet.assets\image-20231101195557612.png)
