...
-   public/ 目录：用于**存放一些公共资源**，如 HTML 文件、图像、字体等，这些资源会被直接复制到构建出的目标目录中。
...
然而，现在的router-view只放在了APP.vue中，没有(我尝试了，但是好像有点复杂)往子组件中放
...
const router = createRouter({
    ...
    routes:[
        ...
        🆕⭐方法二： /showDetail2 就正常写就行 方法2不涉及这里
        {
            path:'/showDetail2',
            components:{
...
+ 方法1：ShowDetail.vue 通过useRoute获取路径参数 《不推荐》
...
-   方法2：ShowDetail2.vue通过useRoute获取键值对参数 《推荐》
```html
<script setup type="module">
    import{useRoute} from 'vue-router'
    import { onUpdated,ref } from 'vue';
...
“http://localhost:5173/#/show1/4/java” 

另一种是：

“http://localhost:5173/#/show2?id=4&lang=C%23”  

 

果断选第二种，因为第二种不用额外配置router.js。第一种在router.js里面还要写乱七八糟的东西。
...
发送是 import {useRouter} from "vue-router"

接受是 import { useRoute } from "vue-router";
...
1.  **全局前置守卫**：在路由**切换前被调用**，可以用于验证用户是否已登录、中断导航、请求数据等。
2.  **全局后置守卫**：在路由**切换之后被调用**，可以用于处理数据、操作 DOM 、记录日志等。
...
```html
    <script>
        ...
        */
        🆕这里的resolve,reject两个参数，是俩函数，调用之后，会导致promise变为成功和失败
        let promise =new Promise(function(resolve,reject){
            🆕开启线程，函数中的"promise success"是参数，会传递给then里面的回调函数
            console.log("promise do some code ... ...")
            //resolve("promise success")
            reject("promise fail")
        })
        console.log('other code1111 invoked')
        //2.获取回调函数结果  then在这里会等待promise中的运行结果,但是《不会阻塞》代码继续运行
        🆕此行代码之内的代码，在promise执行完毕后，才会执行
        🆕里面的写法是固定的，必须是两个function
        promise.then(
            🆕promise转换为resolved后会执行的代码
            function(value){console.log(`promise中执行了resolve:${value}`)},
            🆕promise转换为rejected后会执行的代码
            function(value){console.log(`promise中执行了reject:${value}`)}
        )
        // 3 其他代码执行   
...
#### 9.0.4 Promise catch()  独立接收reject状态
...
<script>
    ...
    🆕这里写成lambda表达式也可以(resolve,reject)=>{}
    ...
    /* 
        ...
        then中的reject对应的回调函数被后续的catch替换后,catch中接收的数据是一个异常对象/以及失败结果
        */
    promise.then(
        function(resolveValue){console.log(...)}
        //function(rejectValue){console.log(...)}
    ).catch(
        🆕此处的catch相当于 promise出现报错+ 结果变为reject
...
> &#x20;async和await是ES6中用于处理异步操作的新特性。通常，异步操作会涉及到Promise对象，而async/await则是在Promise基础上提供了**更加直观和易于使用**的语法。

>  async 用于标识函数的

🆕**await右侧接受的是一个promise对象！！！**

🆕**并且会在接收到对象之前，一直等待**

1. async标识函数后,async函数的返回值会变成一个promise对象

   🆕下面三句是废话，也就是只要加了async，他会尽一切办法把你返回的东西变为promise的返回。默认是成功版本。
...
<script>
    ...
    /* 
        async 用于标识函数的
            1. async标识函数后,async函数的 返回值 会变成一个promise对象
...
<script>
    /* 
            ...
            2. 如果表达式是promise对象,await返回的是promise成功的值
           	   如果失败了,就会抛出异常,可以通过 try ... catch捕获处理
...
<script setup type="module">
  ...
  let getLoveMessage =()=>{
      🆕⭐此处的axios是创立并执行。也就是这一句话表示，调整好了axios的参数，并且去发了请求，返回了一个promise对象。
      后面的直接.then，则是相当于promise为匿名对象，匿名类的调用
    axios({
      method:"post", // 请求方式
      url:"https://api.uomg.com/api/rand.qinghua?format=json",  // 请求的url
      data/params:{ 
        //🆕 当请求方式为post-data，的数据以JSON串放入请求体, 
        //    如果是GET-params则以key=value形式放url后
        
        //ps： params参数中的内容，无论method方法为何，都会放到？之后，变为键值对 
        //username:"123456"
      }
    }).then( function (response){//响应成功时要执行的函数
      console.log(response)
       // Object.assign 两个参数都是放入对象，会将后面对象的属性，赋值给前面的。 前面有啥 放啥
...
**axios返回的是一个promise对象**，可以在其左侧加上 await，来直接获得当前axios的运行结果。
...
// 不知道咋用 axios.post(url[, data[, config]])
...
<script setup type="module">
  ...
  let Data =reactive({code:1,content:'我努力不是为了你而是因为你'})
  ...
  let getLoveWords= async ()=>{
    try{
      return await axios.post(
        'https://api.uomg.com/api/rand.qinghua',
        {//《请求体》中的JSON数据
            //后端使用 SysSchedule schedule = WebUtil.readJson(req, SysSchedule.class);来接
            username:'zhangsan',
            password:'123456'
        },
        {// 其他参数
         	params:{// url上拼接的《键值对》参数
                //🆕后端使用Integer.parseInt(req.getParameter("uid"));来接
            	format:'json',
          	},
          	headers:{// 请求头
            	'Accept' : 'application/json, text/plain, text/html,*/*',
...
> 如果想在axios发送请求《之前》,或者是数据响应《回来》在执行then方法之前做一些额外的工作,可以通过拦截器完成
...
此处意思是 新建了一个js文件，就和之前的路由router文件一样
...
instance.interceptors.request.use(
    ...
    error=>{
        ...
        //类似匿名类 直接创建一个失败的promise，参数error表示赋值
        return Promise.reject(error)
    }
)
// 添加响应拦截器
instance.interceptors.response.use(
    ...
    error=>{
        ...
        console.log(error)
        //🆕此处返回的promise是返回给调用它的 .post 或者.get
        return Promise.reject(error)
    }
)
// 默认导出
...
> 前后端分离模式下,客户端请求前端服务器获取视图资源,然后《客户端自行》向后端服务器获取数据资源,前端服务器的 协议,IP和端口和后端服务器很可能是不一样的,这样就产生了跨域
...
> 前端项目代理模式处理
>
> 🆕也就是不要客户端浏览器去发，而是用服务器去发请求，获得结果，然后再传给浏览器
>
> 但是这样子很复杂，而且耽误服务器的效率
...
> 后端跨域过滤器方式处理
>
> 🆕还是不用服务器了，转而还是使用浏览器去发请求，但是发之前先要发一个跨域请求，得到批准后，一定时间内可以避免同源策略

![1683364436315](images/1683364436315-1690533535580.png)

+ CrosFilter过滤器 《临时使用》<跨域拦截，返回跨域请求成功>
...
//actions 操作的方法  存储数据的复杂业务逻辑方法
export const definedPerson = defineStore(
    {
        id: 'personPinia', //⭐必须全局唯一
        state:()=>{ // 响应式数据 ，固定写法
            return {
                username:'张三',
                age:0,
        ...
        getters:{// 用于定义一些通过数据计算而得到结果的一些方法 一般在此处不做对数据的修改操作
                 // getters中的方法可以当做属性值方式使用，也就是不需要像调用函数一样加括号
            getHobbiesCount(){
...
🆕随后，在哪个组件使用，就在哪个组件引用`definedPerson`即可
...
在router.js中，我们引入自己创作的pinia对象，要在括号里传入一个(pinia)才行。
...
<https://element-plus.gitee.io/zh-CN/component/button.html>VS