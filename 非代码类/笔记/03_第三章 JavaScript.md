...
//        转换对象和字符串⭐
    ...
    {
        String person = "{\"name\":\"张三\",\"age\":27,\"dog\":{\"name\":\"小黄\",\"age\":2}}";
        ObjectMapper objectMapper = new ObjectMapper();
        Person p1 = objectMapper.readValue(person,Person.class);
...
只要是什么东西转成`JSON`（其实就是字符串），就用`objectMapper.writeValueAsString(map);`

想转回去，就用`Person p1 = objectMapper.readValue(person,Person.class);`



经常用来转换的是 `list` ，数组，还有`map`。

其中`map`转完之后，有明显的 `{ XXX }`

而 `list`和数组 转完之后，则是 `[ “XXX” ]`
...
| [join()](https://www.runoob.com/jsref/jsref-join.html)⭐      | 把数组的所有元素放入一个字符串。 参数是分隔符                |
...
| [slice()](https://www.runoob.com/jsref/jsref-slice-array.html)⭐ | 选取数组的一部分，并返回一个新数组。 参数[A,B)               |
| [some()](https://www.runoob.com/jsref/jsref-some.html)       | 检测数组元素中是否有元素符合指定条件。                       |
| [sort()](https://www.runoob.com/jsref/jsref-sort.html)       | 对数组的元素进行排序。                                       |
| [splice()](https://www.runoob.com/jsref/jsref-splice.html)⭐  | 从数组中添加或删除元素。 [A,B,C...]从第A个下标，删除B个元素(为0就是增加)，增加的是c，d，e |
...
| 方法                                                         | 描述                                          |
| :----------------------------------------------------------- | :-------------------------------------------- |
| [toString()](https://www.runoob.com/jsref/jsref-tostring-boolean.html)⭐ | 把布尔值转换为字符串，并返回结果。 返回”true” |
| [valueOf()](https://www.runoob.com/jsref/jsref-valueof-boolean.html) | 返回 Boolean 对象的原始值。 没啥用            |
...
| parseInt,parseFloat()                                        | 把字符串啥的转换为**数字**                           |
...
| [onchange](https://www.runoob.com/jsref/event-onchange.html)⭐ | 该事件在表单元素的内容改变时触发( <input>, <keygen>, <select>, 和 <textarea>)**在失去焦点后检查** |
|                                                              | onchange="testChange(**this.value**)"  配合 var testChange = function (**value**) {XXX} 可以打印出改变后的值 |
...
| [onreset](https://www.runoob.com/jsref/event-onreset.html)⭐  | 表单重置时触发 **绑定在form上 ， 而不是reset按钮**           |
| [onsearch](https://www.runoob.com/jsref/event-onsearch.html) | 用户向搜索域输入文本时触发 ( <input="search">)               |
| [onselect](https://www.runoob.com/jsref/event-onselect.html) | 用户选取文本时触发 ( <input> 和 <textarea>)                  |
| [onsubmit](https://www.runoob.com/jsref/event-onsubmit.html) | 表单提交时触发 ，**绑定在form上 ， 而不是submit按钮**        |
|                                                              | 在函数中加入 `event.preventDefault()`阻止事件默认行为，可以阻止提交 |
...
+ BOM编程的对象结构如下
    ...
    + window 顶级对象,代表整个浏览器窗口
        ...
        + sessionStorage对象      代表浏览器的本地数据会话级存储(临时存储，浏览器自己存的)
...
|                                                              | back()方法，返回上一页（左侧）  ，forward 去下一页（右侧），go(X) 向后(右侧)走x页 |
...
| [location](https://www.runoob.com/jsref/obj-location.html)⭐  | 用于窗口或框架的 Location 对象。请参阅 [Location 对象](https://www.runoob.com/jsref/obj-location.html)。 地址栏 |
|                                                              | location.href = "http://www.baidu.com"; 直接会去新地址，这个href是**属性**，不是方法 |
...
| [sessionStorage](https://www.runoob.com/jsref/prop-win-sessionstorage.html)⭐ | 在浏览器中存储 key/value 对。 在关闭窗口或标签页之后将会**删除**这些数据。 |
|                                                              | getItem(key)  , setItem(key,value)  , removeItem(key)        |
...
|                    | 比如 document.getElementsByTagName(“input”) | 找所有input标签  |
...
| element.appendChild(ele)                 | 将ele添加到element所有子节点后面，不加引号 |
| parentEle.insertBefore(newEle,targetEle) | 将newEle插入到targetEle前面                |
| parentEle.replaceChild(newEle, oldEle)   | 用新节点替换原有的旧子节点                 |
| element.remove()                         | 删除element标签 **谁调用，移出谁**         |
...
| [\w](https://www.runoob.com/jsref/jsref-regexp-wordchar.html)⭐ | 查找数字、字母及下划线。 0-9 a-z A-Z _      |
...
|                                                          | var reg = /o/g  str.replace(reg,”Z”) 全局替换o为Z |
