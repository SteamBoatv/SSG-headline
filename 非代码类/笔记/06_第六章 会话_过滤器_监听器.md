...
> cookie是一种客户端会话技术,cookie**由服务端产生**,它是服务器存**放在浏览器**的一**小份**数据,浏览器以后每次访问该服务器的时候都会将这小份数据携带到服务器去。
...
```
Cookie cookie3 = new Cookie("age","10");
//设置谁能读 但其实是存到了 浏览器里面，而不是存到了servlet中
...
+ 说明
    ...
    + filterChain.doFilter(request,response);在放行时需要传入request和response**,就是之前servlet中的请求对象！**,意味着,这里没有产生新的request和response对象
...
@WebFilter(
        ...
        urlPatterns = {"/servletA","*.html"},⭐ // 使用逗号隔开
...
+ 按监听的对象划分，**需要监听啥功能，你的listener就实现谁**
...
提前总结 ，其实是对三种对象进行监听(ServletContext,Session,request)，监听的范围有两种(监听是否创建和删除，监听参数的增删改查)
...
可以看到，都是使用参数，然后.你要的对应的对象。至于继承谁，看上面3.1概述部分，里面有说。
...
```
    ...
    public void attributeReplaced(ServletContextAttributeEvent scae) {
        ...
        Object value = scae.getValue();
        ServletContext application = scae.getServletContext();
        Object newValue = application.getAttribute(name);//⭐ 这样子才是最新的
        System.out.println("application"+application.hashCode()+" change:"+name+"="+value+" to "+newValue);
    }
```

可以看到，这里面是参数.getName，.getValue。

特殊情况是，在查看修改的参数时，.getName是获得旧的数据，而先获得当前变量，再去变量里面获得getAttribute，才是新值。这点稍加留意即可。



### 3.2.1 application域监听器

> ServletContextListener  监听ServletContext对象的创建与销毁 没啥用！！！
...
> ServletContextAttributeListener 监听ServletContext中属性的添加、移除和修改 有用！！！
...
+ ServletContextAttributeEvent对象代表属性变化事件，它包含的方法如下：有用！！！
...
<script>
  function loadXMLDoc(){
      //这个XMLHttpRequest没有出现在其他地方，就在这用⭐
    var xmlhttp=new XMLHttpRequest();
      // 设置回调函数处理响应结果
    xmlhttp.onreadystatechange=function(){
      if (xmlhttp.readyState==4 && xmlhttp.status==200)
      {
          //这里的 responseText接收到的是 后端servlet的“resp.getWriter().write("hello:" + username);”
          //resp write是写给前端的。
          //之前涉及到 resp 都是sendRdirect
        ...
        function checkUsername(){
            ...
            request.onreadystatechange= function (){
                ...
                if(request.readyState == 4  && request.status== 200){
                    ...
                    //  判断业务码是否是200，这里的200是自己在枚举类中定义的success，
                    //而不是浏览器默认的，之前的request.status== 200的200
...
public class Result<T> {
    ...
    public Result(){}
    // 返回数据
    //静态方法的<T> 已经和class的<T>无关了
    protected static <T> Result<T> build(T data) {
        //快速构建一个result对象，并返回。
        //此result对象中，只存在data数据
    ...
    public static<T> Result<T> ok(T data){
        ...
        //使用了第三种build重载
        //先创建一个result，传入data。然后默认往里放成功的枚举类型
